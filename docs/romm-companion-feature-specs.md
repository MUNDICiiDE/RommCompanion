...
> **Implementation status (2026-09-01):** Feature 6 is complete. IPC schema v18 and SQLite schema v11 provide normalized origin-scoped library records, exact view pages, full details, source/staleness reporting, authenticated artwork retrieval, a bounded 1 GiB artwork LRU, and authoritative remote-deletion reconciliation. Stable 48-item paging, 107-ROM completeness, 10,000-ROM state responsiveness, Home shelves, typed views, controller-safe search/filter/detail flows, immediate offline discovery, live/local/cache timestamps, distinct empty/error states, retained-content retry, lazy covers, accessible placeholders, and one corrupt-image refresh are implemented and verified. Download/favorite mutations remain owned by Features 9 and 7.
...
- [x] **6.5 Offline, empty, error, and performance validation** — Add stale/source indicators, cached fallbacks, retry/empty states, authenticated lazy artwork with safe LRU caching/placeholders, remote-deletion reconciliation, and the 10,000-ROM responsiveness checks.
...
IPC v18 exposes `listLibrary { query, limit, offset }`, `getGameDetails { romId }`, `getArtwork { romId, preferredKind, refresh }`, legacy `listRoms` for the onboarding refresh path, and `getLibraryMetadata`. `LibraryQuery` permits `all`, `recent`, `favorites`, `platform`, `collection`, `smart_collection`, `downloaded`, and `active_downloads`; only the three ID-scoped kinds accept a positive `id`. Optional discovery fields are `search`, `platformId`, paired `collectionId`/`collectionKind`, `favoriteOnly`, `downloadedOnly`, and `sort: id | title | recently_added | release_date`. Search is trimmed, limited to 200 characters, and translated to RomM's `search_term`; filters use `platform_ids`, `collection_id` or `smart_collection_id`, and `favorite`; sort uses RomM's `name`, `id`, or `first_release_date` ordering. The v1 RomM cursor is `{ offset: u64, limit: 48 }`; page results include `items`, optional `total`, `hasMore`, `source: live | cache`, `refreshedAtMs`, and `stale`. Downloaded, active-download, and `downloadedOnly` queries are answered from local state and never contact RomM. The raw RomM adapter disables character, filter-value, and ROM-ID indexes on page reads because those indexes are not used to render cards. `getGameDetails` calls `GET /api/roms/{id}` and returns normalized metadata, files, collections, sibling ROMs, save/state/screenshot counts, manual/soundtrack availability, favorite/local status, and source/staleness. Artwork requests are agent-owned: they use the selected origin and stored token, accept RomM-hosted resource paths only, reject traversal and active content, cap responses at 8 MiB, and return validated PNG, JPEG, WebP, GIF, or AVIF bytes as a data payload.

SQLite schema v11 uses `library_rom`, `library_user_rom`, `library_artwork`, `library_page`, `library_platform`, `library_collection`, `library_snapshot`, `library_game_detail`, and the shared `cache_entry` table. The v11 availability flag supports authoritative full-catalog reconciliation without deleting downloaded content. `library_page` keys include normalized server origin plus the complete typed query key, offset, and limit, so pages from different shelves, searches, sorts, and filter combinations cannot collide; v8 pages migrate to the `all` view. Standard and smart collection IDs are disambiguated by collection kind. Local downloaded state and active download IDs are joined when pages and details are read instead of being copied into disposable remote payloads. When an exact filtered page is unavailable during a retryable outage, the cache evaluates the same tokenized case-insensitive search and filter rules over known normalized records. The v7 `rom_cache` remains only as a backward-compatible onboarding snapshot and migration source. Artwork cache rows are origin-keyed, access-touched, pruned oldest-first to the current fixed 1 GiB budget, and safely evicted when a file is missing or corrupt.
...
- Milestone 6.5 acceptance: visible library data identifies live RomM, local-device, recent cache, and stale cache sources with refresh time; retryable failures preserve prior rows and expose a controller-focusable retry; no-result filters can be cleared separately from true empty/local-empty states; covers load only near the viewport through authenticated Rust requests, never expose the bearer token to the WebView, validate raster signatures and an 8 MiB cap, persist in the origin-keyed LRU, recover from missing/corrupt files, retry decode failure once, and retain an accessible placeholder; a complete authoritative refresh hides removed remote-only ROMs while preserving downloaded copies as `Unavailable on server`; the 10,000-ROM state benchmark stays below two seconds; IPC v18/TypeScript parity, SQLite v11 migration, Rust/frontend tests, strict Clippy, type checking, formatting, Markdown lint, and the production build pass.
...
> **Implementation status (2026-09-05):** Feature 7 is complete through Milestone 7.4. IPC schema v21 and SQLite schema v13 provide typed online mutation, one origin-scoped durable pending favorite per ROM, and an authentication-time reconciliation summary. Queued changes replay in creation order after reconnect, pairing, or manual-token authentication; normalized cache and already-open UI snapshots settle from each typed outcome without allowing an older in-flight response to win. `401` and retryable failures preserve and pause the queue, while `403`, `404`, and other terminal rejections restore the recorded base value and remove the rejected intent.
...
Favorite can be toggled from a card context action or game details. The icon updates immediately. Retryable offline changes show a queued badge; terminal failures revert visibly and offer retry.

### Delivery milestones

- [x] **7.1 Favorite model and online API** — Add cached server state, typed read/write operations, authoritative responses, and permission/error classification.
- [x] **7.2 Optimistic UI** — Toggle immediately from cards/details, serialize per-ROM writes, replace with server truth, and visibly roll back terminal failures.
- [x] **7.3 Offline mutation journal** — Persist one final desired state per ROM, coalesce rapid toggles, survive restart, and expose a queued indicator.
- [x] **7.4 Reconnect reconciliation and validation** — Replay in order, handle refresh races and 401/403/404 outcomes, then pass online/offline/controller acceptance tests.
...
- Resolve favorites through the authenticated user's private standard collection with `is_favorite: true`; ignore another user's public favorite collection.
- Create that private collection only when adding the first favorite and no such collection exists.
- Use RomM 5.x atomic `POST /api/collections/{id}/roms` and `DELETE /api/collections/{id}/roms` membership operations so unrelated favorites are never replaced by a stale full-list write.
- On success, replace optimistic state with the authoritative collection membership returned by RomM.
- On reconnect, submit queued changes in creation order and refresh affected user-ROM data.

### Data and interfaces

IPC v21 exposes `setFavorite { romId, desired }` and returns either `favoriteUpdated { result }` for server-confirmed state or `favoriteQueued { mutation }` for a durable local intent. The queued mutation includes the ROM ID, final desired value, base favorite value and update time, original queue time, and latest coalescing time. A successful authentication response includes the resulting connection state and, when work was queued for that normalized origin, a typed reconciliation summary with queued, attempted, applied, discarded, and remaining counts plus per-ROM applied, discarded, or pending outcomes and an optional pause error. RomM 5.x favorite reads require `collections.read`; mutations require `collections.write`. SQLite v13 adds an origin/operation/ROM unique pending-journal key plus `library_user_rom.favorite_pending`; queue writes update normalized card/detail state and invalidate favorite-dependent exact pages in the same transaction. Agent status reports the origin-scoped pending-favorite count. Local logout restores cached base values and deletes pending favorite mutations for that signed-out profile.

### Failure handling

- 401 pauses the queue for re-pairing; 403 reverts the optimistic change and identifies the missing `collections.write` scope.
...
- Milestone 7.1 acceptance: own-account favorite collection discovery ignores other users' public collections; add and remove use atomic RomM membership routes; absent Favorites is a no-op for removal and is created only for addition; authoritative membership updates normalized card/detail/user/collection cache records and invalidates favorite-dependent exact pages; 401, 403, 404, terminal 4xx, retryable network/5xx, malformed responses, and server disagreement have distinct typed outcomes; IPC v19/TypeScript parity, SQLite v12 migration, Rust tests, strict Clippy, type checking, formatting, Markdown lint, and the production build pass.
- Milestone 7.2 acceptance: card stars and the controller-first game-details action update every visible snapshot immediately; rapid changes for one ROM are sent serially while different ROMs may update concurrently; pending state remains visible; an authoritative response replaces the optimistic value; terminal online failures restore the prior value and expose a controller-, keyboard-, and pointer-accessible retry; a detail response racing a favorite mutation cannot overwrite the newer UI state; frontend tests, type checking, Markdown lint, and the production build pass. Persistent retryable/offline queuing remains explicitly owned by 7.3.
- Milestone 7.3 acceptance: the first retryable favorite failure becomes a durable queued response and moves the agent offline; later offline toggles skip the network timeout; any number of changes for one origin/ROM produces exactly one journal row with the original base and queue time but final desired value; another origin remains isolated; normalized cards/details retain the desired value and pending flag across cache refresh and process restart; the UI distinguishes saving, queued, and terminal failure states and reports the queued count; authoritative success clears the row and pending flag atomically; local logout restores the base and removes the account-bound queue; IPC v20/TypeScript parity, SQLite v13 migration, Rust/frontend tests, strict Clippy, type checking, formatting, Markdown lint, and the production build pass. Automatic reconnect replay remains explicitly owned by 7.4.
- Milestone 7.4 acceptance: successful reconnect, pairing, or manual-token authentication replays the selected origin's queue in original creation order under the library request gate; each successful response refreshes normalized user-ROM and favorite-collection state and atomically clears that intent; typed IPC v21 outcomes update already-open cards and details and invalidate older in-flight UI mutations; `401` and retryable network/`5xx` failures preserve the current and later rows and expose `unauthorized` or `offline`; `403` restores all affected base values, clears intents that cannot succeed with the token, exposes `scope_error`, and identifies `collections.write`; `404` restores and clears the affected favorite while marking the remote ROM unavailable; other terminal `4xx` outcomes restore and clear only the rejected mutation; the existing controller-focusable reconnect action is sufficient to retry without adding a separate sync command; Rust/frontend tests, strict Clippy, TypeScript checking, formatting, Markdown lint, production build, and rebuilt debug/release agent binaries pass.

## 8. EmuDeck detection and platform mappings

> **Implementation status (2026-09-07):** Milestones 8.1 through 8.4 are complete, and 8.5 implementation plus automated qualification are complete. IPC schema v24 gives every newly detected mapping a UUID and reports preset upgrades with changed and preserved-custom fields. Rescans merge by RomM platform ID: missing mounts retain the accepted mapping, custom fields are never replaced, and only fields still owned by the same newer preset advance. SQLite schema v14 enforces one current row per server-origin/platform pair while retaining any pre-existing duplicate as disabled `superseded` history; accepted mappings round-trip from normalized rows rather than depending on wizard draft JSON. Windows, Steam Deck/microSD, symlink, missing/remounted storage, overlap, origin-isolation, migration, and unintended-write behavior have automated fixture coverage. The physical Steam Deck/removable-media qualification portion of 8.5 remains deferred until that hardware environment is available.
...
Detection presents proposed mappings with a confidence label and source. The user reviews each enabled platform and may browse or type different ROM/save/state paths. Scanning, browsing, typing, validation, persistence, and automatic rechecks never create or watch directories. Check/Save uses non-mutating OS directory-access checks and never opens or changes a ROM, save, or state file. A separate confirmation may create exactly one named child beneath the currently browsed existing parent; the created folder must still be selected and the mapping saved.
...
- [x] **8.3 Controller-complete mapping editor** — Review, enable, browse/type, override every ROM/save/state path, choose archive policy, and confirm intentional directory creation.
- [x] **8.4 Path and removable-storage safety** — Validate absolute/canonical paths, access, space, overlaps, symlinks, duplicate destinations, mounts, and automatic remount recovery.
- [ ] **8.5 Persistence, preset upgrades, and validation** — **Implementation and automated qualification complete; physical Steam Deck/removable-media qualification pending.** Preserve custom fields across detection/preset changes and pass Steam Deck, Windows, removable-media, and unintended-write tests.
...
- Merge a rescan with accepted rows by RomM platform ID. Preserve the local mapping ID, enabled choice, accepted path when its mount is absent, and every field whose `customFields` entry is true.
- Apply a preset upgrade only when its ID matches and its version increases. Advance only preset-owned ROM/save/state roots, archive policy, and filename strategy; report both updated and preserved custom fields to the UI.
...
- Validate absolute path, parent existence, read/write access, free space, overlapping roots, and whether removable storage is currently mounted. Manual validation and save request directory-list/create/remove rights without writing; the 15-second background recheck uses filesystem metadata for write capability.
...
`PlatformMapping` contains local UUID, normalized server origin, RomM platform ID, preset ID/version, ROM root, save roots, state roots, archive policy, filename strategy, enabled flag, validation status, and `isCustom` flags per field. Accepted selections are updated transactionally: current rows for the same origin are disabled first and submitted drafts are then upserted by origin/platform; rows are never deleted by selection changes. SQLite schema v14 adds one current origin/platform constraint. If a pre-v14 database contains duplicates, the newest remains current and every older row is retained as disabled `superseded` history.

IPC v24 uses `detectMappings`, `getMappingDrafts`, `saveMappingDrafts`, `validateMappings`, `recheckMappings`, and `saveMappings` for the mapping flow. `MappingDetectionResult.presetUpdates` identifies the mapping/preset versions plus preset-owned fields updated and custom fields preserved. `validateMappings` and `saveMappings` perform non-mutating OS access-right checks; `recheckMappings` is a lightweight read-only check. `MappingPathValidation` reports the draft/field, entered and canonical path, `ready | temporarily_unavailable | permission_denied | unsafe`, read/write capability, available bytes, removable/mounted flags, and symlink presence. `browseDirectories { path? }` returns the current path, parent, up to 500 sorted subdirectories, symlink indicators, and whether the response is a location list. `createDirectory { parentPath, name, confirmed }` rejects an unconfirmed request, relative or unavailable parent, separators/traversal, names longer than 128 characters, and file collisions; it calls single-level directory creation rather than recursive creation. Detection, browsing, validation, and automatic rechecks never write.
...
- A rescan with an unavailable preset mount retains the accepted path and lets health validation mark it unavailable; it never substitutes an internal or similarly named directory.
- Corrupt persisted mapping JSON or unknown enum values fail loading explicitly. A preset upgrade failure leaves the prior transaction intact.
...
- Milestone 8.3 acceptance: each enabled platform exposes a typed and browsable ROM root plus add/edit/remove controls for zero or more save and state roots; all controls and the modal browser remain reachable by controller, keyboard, and pointer and restore the invoking Browse action on close; location, parent, directory, use-current, new-folder, confirmation, retry, and cancel actions remain inside the modal focus boundary; creation requires a second explicit action, accepts one final component only, never creates missing parents, and leaves the draft unchanged until the user chooses the resulting folder; blank optional rows are removed before save; IPC v22/Rust/TypeScript parity, filesystem tests, frontend tests, strict Clippy, formatting, Markdown lint, and production builds pass. Access/free-space/overlap/symlink/mount validation remains explicitly owned by 8.4.
- Milestone 8.4 acceptance: each enabled ROM/save/state root is resolved canonically, listed, checked for create/remove rights without writing, measured for free space, classified for removable/mounted/link state, and returned through IPC v23/TypeScript parity; canonical duplicates and any root overlap block saving; an unavailable root is `temporarily_unavailable` rather than redirected; accepted mapping statuses are rechecked read-only every 15 seconds and recover to `valid` after remount; the controller-focusable Check action exposes actionable per-root results; filesystem tests prove validation leaves roots unchanged and cover canonical results, overlaps, missing roots, and persisted remount recovery; Rust/frontend tests, strict Clippy, type checking, formatting, Markdown lint, and production builds pass. Preset-upgrade preservation and physical removable-media qualification remain explicitly owned by 8.5.
- Milestone 8.5 automated acceptance: initial detections use distinct UUIDs; saved mappings round-trip every preset/custom field from normalized SQLite rows and remain isolated by server origin; rescans preserve IDs, enabled mappings, custom fields, and missing removable destinations; same-preset version increases update only preset-owned fields and produce typed IPC v24 summaries; schema v14 coalesces conflicting current rows without deleting their historical configuration; Windows path-case, Steam Deck/microSD aliases, Linux mount/symlink, unavailable/remounted media, and zero-validation-write fixtures pass. Physical Steam Deck insertion/removal and end-to-end external-frontend placement remain required before checking 8.5 and marking Feature 8 complete.
...
- [ ] **11.1 Persistent metadata and local-state cache** — **In progress:** Feature 6 supplies origin-scoped ROM/platform/collection/user/artwork records, exact page/detail snapshots, downloaded/active-download joins, timestamps, availability reconciliation, and immediate offline search/filtering; queue-wide facts and reset/recovery controls remain.
- [ ] **11.2 Artwork LRU cache** — **In progress:** Feature 6.5 adds authenticated progressive storage, access tracking, fixed 1 GiB oldest-first pruning, bounded raster validation, and missing/corrupt-file recovery; configurable 256 MiB-to-10 GiB limits, pin tracking, startup enforcement, status, and safe clearing remain.
- [ ] **11.3 Offline library and search UX** — **In progress:** Cached browse/search/filter/detail flows, live/local/cache timestamps, 24-hour stale labels, accessible cover fallbacks, and retained-content retry are complete; the FTS index plus removal/queue offline operations remain.
